...
'''
    ...
    In this problem, you will implement the Elo rating algorithm to rank players based upon match results (win/lose). 
    Assuming all the players' performance are random following a Logistic distribution with a scale of 400
...
'''
    ...
    ---- Inputs: --------
        ...
        * K: k-factor, a constant number which controls how maximum amount of change that can be applied on the rating 
        of a player based upon the result of one game.
    ---- Outputs: --------
...
def compute_ratings(G, n, K=16):
    ...
    for index, row in G.iterrows(): # for each game, update the ratings based upon the result of the game
        ...
        ## INSERT YOUR CODE HERE (5 points)
        RA, RB = R[A], R[B]
        EA, EB = compute_EA(RA, RB), (1-compute_EA(RA, RB))
        R[A], R[B] = update_RA(RA,SA,EA,K), update_RB(RB,SB,EB,K)
        ########################################
    return R
    #-----------------
    '''  
...
* R:  the ratings of all the n players, a numpy array of length n, where R[i] represents the current rating of the i-th player. 
* G:  a dataframe of all the game results, a pandas data frame of shape (m by 2), where each row of the matrix G[i] represents the result of the i-th game, 
and G[i] = (A,B) contains the IDs of the winning team (A) and losing team (B) in the game. 
