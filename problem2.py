...
'''
    Problem 2: Build FaceMash (20 points)
     In this problem, we will build a webpage similar to FaceMash using Flask. We use 8 photos in the static folder as 
     an example dataset. Each time when a user visit the website, we will randomly sample two images to show, and users 
     can click to vote on the photo
...
'''
    (Random Sample) Given the total number of photos (n), randomly choose a pair of the photo IDs (A and B) to return.
     Here we assume the IDs of all the photos are: 0,1,2,..., n-1. For example, if we have 5 photos (n=5), the IDs of 
     the photos will be: 0,1,2,3,4. We then randomly sample two IDs (such as A=2, B=0), where the two IDs should be 
...
def sample_pair(n):
    ...
    if n == 1:
        raise NameError("Not Enough Photos")
...
#----------------------------------------------------
'''
    (Homepage) Create the homepage of facemash. Each time when a user visit the URL '/', randomly sample two photo IDs 
    (A and B), and use these IDs and the template to render the webpage. Here the 'template' variable (a global variable) 
    was already created in line #5 of this file. You could use this template to render the webpage using the IDs (A and B). 
    ---- Outputs: --------
...
def facemash():
    ...
    a,b = sample_pair(8)
    webpage = load_template('index.html').render(A=a, B=b)
    #########################################
    return webpage
    #-----------------
    '''  
...
#----------------------------------------------------
'''
    (Vote Page) Create a page for user to vote. For example, if a user visits (/vote/4/3), it means that the user was 
    given two photos with ID 3 and 4. This user voted Photo 4 wins, Photo 3 loses. In this function, please (1) update 
    the game result into the list (data), then (2) redirect the page to the homepage of facemash. 
	 (1) In line #6 of this file, we have created an empty list (called 'data') to store all the vote data. Please store 
	 the game result by add a tuple of (3,4) into the list (data). 
